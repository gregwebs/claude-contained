...
- **Optional Java layer**: `INCLUDE_JAVA_LAYER` build arg (default `true`) gates JBR, HotswapAgent, jdtls, Maven, and JBang installation in the Dockerfile. Each Java-related `RUN` step checks the arg and exits early (or skips its heredoc) when disabled, rather than being split into separate Dockerfile stages. `JAVA_HOME`/`PATH` env vars and the `~/.m2` mount still reference Java paths unconditionally when disabled — harmless no-ops since the referenced binaries/dirs simply don't exist. The devcontainer (Java/Spring focused) requires the image built with this layer included.
