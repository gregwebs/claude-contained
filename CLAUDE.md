...
- **image/** - Files copied into the image, kept out of the Dockerfile so it stays under Apple Containers' 16k file limit:
  - `image/entrypoint.sh` - Configures `host.local` for host service access, matches host UID/GID, sets up path parity, protects `.git/config`, sets up the egress allowlist firewall (opt-in, before the privilege drop), starts Xvfb, and drops to the `dev` user.
  - `image/egress-firewall.sh` - Builds the nftables/dnsmasq egress allowlist firewall when `CONTAINED_EGRESS_ALLOW` is set. Runs as root, invoked by entrypoint.sh. See "Egress allowlist" below.
  - `image/sni-proxy.py` - Root-owned passthrough proxy the firewall NATs outbound 443/80 into; reads the TLS SNI or HTTP Host header and dials out by that name, never by the original destination.
...
- **Egress allowlist** (opt-in, hard security boundary — see `docs/adr/0001-in-container-dns-snooping-egress-firewall.md`): `--allow-host HOST` (repeatable, also `--allow-host=HOST`) / `CLAUDE_ALLOW_HOSTS` (comma-separated default, flag wins, same precedence as `--dns`/`CLAUDE_DNS`) restricts the container's `dev` user to a strict hostname allowlist. `merge_allow_hosts()` (in both launcher scripts, unit-tested in `tests/egress-allowlist.test.sh`) combines the selected tool's `default_allow_hosts_for_tool()` base list (API + auth only, skippable via `--no-default-hosts`) with the explicit entries, de-duplicated, and passes the result to the container as `CONTAINED_EGRESS_ALLOW` plus `--cap-add NET_ADMIN --cap-add NET_RAW`. `CLAUDE_REQUIRE_ALLOWLIST=1` makes the launcher refuse to start at all if nothing ends up in the allowlist. Enforcement is entirely inside the container (`image/egress-firewall.sh`, root, before the `dev` privilege drop): dnsmasq resolves only allowlisted names (NXDOMAIN otherwise) and live-feeds their IPs into an nftables set, while `image/sni-proxy.py` — NATed in front of `dev`'s outbound 443/80 — reads the TLS SNI or HTTP Host header and dials out by that name rather than the client's original destination, closing the shared-CDN-IP bypass without TLS interception. `host.local` and literal IP/CIDR entries bypass the proxy and connect directly; only they accept an optional `:PORT` scope (DNS-name entries are always all-ports, and match subdomains). Root is never filtered — it's the enforcement plane, not something the boundary applies to — so `STAY_ROOT=1` combined with an active allowlist is refused. Setup failing for any reason aborts the container rather than starting with the allowlist silently unenforced.
...
- **Egress allowlist scope**: `--allow-host` entries only accept DNS names or IPv4 (IPv6 dev egress is blocked outright, not selectively allowlisted). On `claude-docked`, the boundary is weaker than on `claude-contained` since Docker shares the host kernel instead of running each container in its own VM.
