...
func TestAttachHoldsNoWorktreeLock(t *testing.T) {
	...
	if !strings.HasSuffix(got, "aic-live /usr/local/bin/tool-env /usr/local/bin/srt-run /usr/local/bin/shell-run") {
		t.Errorf("argv = %q, want it to end with %q", got, "aic-live /usr/local/bin/tool-env /usr/local/bin/srt-run /usr/local/bin/shell-run")
	}
}
